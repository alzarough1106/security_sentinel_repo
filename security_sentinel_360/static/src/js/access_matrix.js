...
class AccessMatrix extends Component {
    ...
    async loadFilterOptions() {
        try {
            const opts = await rpc("/security_sentinel_360/matrix_filter_options", {});
    ...
    async loadMatrix() {
        ...
        try {
            const data = await rpc("/security_sentinel_360/matrix_data", {
                options: {
            ...
            this.notification.add("Error loading matrix: " + (e.message || e), { type: "danger" });
        }
    ...
    async openExplainModal(userId, modelId) {
        ...
        try {
            const data = await rpc("/security_sentinel_360/matrix_explain", {
    ...
    async runCompare() {
        ...
        m.loading = true;
        try {
            m.data = await rpc("/security_sentinel_360/matrix_compare", {
    ...
    async runFind() {
        ...
        m.loading = true;
        try {
            m.data = await rpc("/security_sentinel_360/matrix_find_users", {
