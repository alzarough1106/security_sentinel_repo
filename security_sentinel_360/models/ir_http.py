...
class IrHttp(models.AbstractModel):
    ...
    def _dispatch(cls, endpoint):
        ...
        cls._track_session()  # <-- runs on literally every request
        return result

    @classmethod
    def _is_background_request(cls):
        path = (request.httprequest.path or "")
        return path.startswith(cls._ACTIVITY_IGNORE_PREFIXES)
    ...
    def _track_session(cls):
        ...
        if getattr(request.session, "should_rotate", False):
            return
        if cls._is_background_request():          # <-- key fix
            return
        if "res.users.session" not in request.env:
            return
