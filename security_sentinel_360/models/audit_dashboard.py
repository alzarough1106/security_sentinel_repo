...
class AuditDashboard(models.AbstractModel):
    ...
    def get_dashboard_data(self):
        ...
        # Safely convert datetimes to strings for the ORM domains
        since_7d = fields.Datetime.to_string(now_dt - timedelta(days=7))
        since_30d = fields.Datetime.to_string(now_dt - timedelta(days=30))
        today_str = fields.Date.to_string(today_date)
        ...
        try:
            kpis = {
                ...
                "new_devices_today": Session.search_count([
                    ("is_new_device", "=", True), ("login_date", ">=", f"{today_str} 00:00:00"),
                ]),
                "logs_today": Log.search_count([("create_date", ">=", f"{today_str} 00:00:00")]),
            ...
            sessions_by_state = {
                ...
                "data": [r.get("state_count", 0) for r in state_rows],
            }
            ...
            actions_by_type = {
                ...
                "data": [r.get("method_count", 0) for r in method_rows],
            }

            # Activity Trend
            days = [(today_date - timedelta(days=i)) for i in range(13, -1, -1)]
            daily_counts = [
                Log.search_count([
                    ("create_date", ">=", f"{fields.Date.to_string(day)} 00:00:00"),
                    ("create_date", "<=", f"{fields.Date.to_string(day)} 23:59:59"),
                ]) for day in days
            ]
            activity_trend = {
            ...
            user_rows = Log.read_group(
                [("create_date", ">=", since_30d)], ["user_id"], ["user_id"]
            )
            user_rows = sorted(user_rows, key=lambda r: r.get("user_id_count", 0), reverse=True)[:5]
            top_users = {
                "labels": [r.get("user_id")[1] if r.get("user_id") else "System/Unknown" for r in user_rows],
                "keys": [r.get("user_id")[0] if r.get("user_id") else False for r in user_rows],
                "data": [r.get("user_id_count", 0) for r in user_rows],
            }

            # Top Models
            model_rows = Log.read_group(
                [("create_date", ">=", since_30d)], ["model_name"], ["model_name"]
            )
            model_rows = sorted(model_rows, key=lambda r: r.get("model_name_count", 0), reverse=True)[:5]
            top_models = {
                "labels": [r.get("model_name") or "Unknown" for r in model_rows],
                "keys": [r.get("model_name") for r in model_rows],
                "data": [r.get("model_name_count", 0) for r in model_rows],
            }
