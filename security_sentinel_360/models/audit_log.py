...
class AuditLog(models.Model):
    ...
    model_name = fields.Char(
        string="Technical Model Name", related="model_id.model", store=True
    )
    ...
    def _field_label(self, field_name):
        ...
        field = (
            ...
            .sudo()
            .search([("model", "=", self.model_name), ("name", "=", field_name)], limit=1)
        )
    ...
    def _compute_changes(self):
        for rec in self:
            ...
            rec.changes_detail = (
                ...
                if detail_rows
                else "<p>No detailed changes available.</p>"
            )
    ...
    def _get_current_session(self):
        if request and request.session and request.session.sid:
            return (
                self.env["res.users.session"]
                .sudo()
                .search(
                    [("session_token", "=", request.session.sid), ("state", "=", "active")],
                    limit=1,
                )
            )
        return False
    ...
    def create_log(self, model_name, res_id, method, changes=None, record_name=False):
        ...
        try:
            ...
            payload = str(changes)
        self.sudo().create({
            "user_id": self.env.uid or SUPERUSER_ID,  # Fallback imported correctly
    ...
    def _cron_purge_old_logs(self, retention_days=None):
        if retention_days is None:
            retention_days = int(
                ...
                .sudo()
                .get_param("security_sentinel_360.log_retention_days", default=365)
            )
        if retention_days <= 0:
            return
