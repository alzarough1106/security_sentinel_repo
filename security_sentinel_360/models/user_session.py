...
class ResUsersSession(models.Model):
    ...
    def _invalidate_disk_session(self, token):
        """Clears auth tokens from disk session while keeping db context intact to avoid 404 nodb errors."""
        try:
            stored_session = root.session_store.get(token)
            if stored_session:
                db = stored_session.get("db")
                stored_session.clear()
                if db:
                    stored_session["db"] = db
                root.session_store.save(stored_session)
        except Exception:
            pass
    ...
    def action_kill_session(self):
        ...
        for rec in self:
            if rec.state != "active":
                continue
            is_current = bool(request and getattr(request, 'session', None) and request.session.sid == rec.session_token)
            if not is_current:
                self._invalidate_disk_session(rec.session_token)
            else:
                killed_own_session = True
            rec.write({"state": "killed", "logout_date": fields.Datetime.now()})

        if killed_own_session and request and getattr(request, 'session', None):
            db_name = request.session.db
            request.session.logout(keep_db=True)
            if db_name:
                request.session.db = db_name
            root.session_store.save(request.session)
            return {"type": "ir.actions.act_url", "url": f"/web/login?db={db_name}" if db_name else "/web/login", "target": "self"}
        return True
    ...
    @api.model
    def _cron_kill_idle_sessions(self):
        param = self.env["ir.config_parameter"].sudo().get_param(
            "security_sentinel_360.idle_timeout_minutes", "60"
        )
        try:
            idle_minutes = int(param)
        ...
        for session in idle_sessions:
            self._invalidate_disk_session(session.session_token)

        idle_sessions.write({"state": "killed", "logout_date": fields.Datetime.now()})

    @api.model
    def _cron_purge_old_sessions(self):
        param = self.env["ir.config_parameter"].sudo().get_param(
            "security_sentinel_360.log_retention_days", "90"
        )
        try:
    ...
    def _cron_dedupe_recent_sessions(self):
        ...
        for row in self.env.cr.dictfetchall():
            ...
            dupe_records = self.browse(dupe_ids)
            for session in dupe_records:
                self._invalidate_disk_session(session.session_token)
            dupe_records.unlink()
