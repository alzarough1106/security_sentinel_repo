...
class ResUsersSession(models.Model):
    ...
    def _send_new_login_email(self):
        ...
        if not template:
            _logger.warning("New-session email template not found (security_sentinel_360.mail_template_new_session)")
            return
        if not self.user_id.email:
            _logger.info("Skipping new-login email for user %s: no email address set", self.user_id.login)
            return
        try:
            template.sudo().send_mail(self.id, force_send=True, raise_exception=True)
        except Exception:
            _logger.exception("Failed to send new-login email for session %s", self.id)
